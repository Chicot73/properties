window.addEventListener('DOMContentLoaded', () => {
    ...
    const emailSelector = form.querySelector('input[inputmode="email"]'); //находим объект по тегу input с атрибутом "inputmode"
    const emailValid = new Inputmask({
        mask: "*{1,20}[.*{1,20}][.*{1,20}][.*{1,20}]@*{1,20}[.*{2,6}][.*{1,2}]", //маска вида "__@__" ,где до "собаки" может быть до 4 полей до 20 симв., разделенных точками, а после - также поле до 20 симв., субдомен (от 2 до 6 симв., домен (2 симв.). 
        greedy: false, //параметр "жадность" - при включении этой функции в маске ставится шаблон "_" под каждый символ из всех 20 возможных в каждое поле! По умолчанию она выключена, и в маске ставится шаблон под всего один символ до и после "собаки", что уменьшает саму маску и делает ее читаемой.
        ...
        definitions: {
            '*': {
                validator: "[0-9A-Za-z_-]",//валидация пропускает только числа от 0 до 9, английские буквы в нижнем и верхнем регистрах, подчеркивание и дефис
                //validator: "[0-9A-Za-z!#$%&'*+/=?^_`{|}~\-]",
                casing: "lower" //автоматически превращает прописные буквы в строчные
            }
        }
    });

    emailValid.mask(emailSelector); //применяем сформированную маску для указанного инпута emailSelector
